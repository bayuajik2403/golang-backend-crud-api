...
func (server *Server) GetTransactionByUser(w http.ResponseWriter, r *http.Request) {
	...
	transactionReceived, err := transaction.FindTransactionByUser(server.DB, pid)
	if err != nil {
		responses.ERROR(w, http.StatusInternalServerError, err)
		return
	}
...
func (server *Server) UpdateTransaction(w http.ResponseWriter, r *http.Request) {
	...
	if uid != transactionUpdate.BuyerID {
		responses.ERROR(w, http.StatusUnauthorized, errors.New("Unauthorized"))
		return
	}
...
func (server *Server) DeleteTransaction(w http.ResponseWriter, r *http.Request) {
	...
	responses.JSON(w, http.StatusOK, "Transaction Deleted")
}
